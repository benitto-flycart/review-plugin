...
class ReviewFormWidget {
    ...
    private initShadowDOM() {
        ...
        this.dialog = this.shadowRoot.getElementById('review_form_dialog') as HTMLDialogElement;
    ...
    private loadInitialData() {
        const dialog = this.shadowRoot.querySelector('#review_form_dialog') as HTMLElement;
        ...
        this.jquery.ajax(this.review_form_store_config.ajax_url, {
            method: 'POST',
            data: {
    ...
    private validateReviewContent(): boolean {
        ...
        const reviewTextError = this.shadowRoot.querySelector('.r_rfw_review_text_error') as HTMLElement;

        if (!isValid && reviewTextError) {
            this.jquery(reviewTextError).html('Review Text Required').removeClass('r_rfw_hide');
        } else {
    ...
    private validateReviewerInfo(): boolean {
        ...
        const validEmailFormat = this.validateEmail(this.review_details.email.trim());

        const isValid = firstNameValid && lastNameValid && emailValid && validEmailFormat;

        if (!isValid) {
            const firstnameDom = this.shadowRoot.querySelector('.r_rfw_review_info_first_name_error') as HTMLElement;
            const lastnameDom = this.shadowRoot.querySelector('.r_rfw_review_info_last_name_error') as HTMLElement;
            const emailDom = this.shadowRoot.querySelector('.r_rfw_review_info_email_error') as HTMLElement;

            if (!firstNameValid) {
                this.jquery(firstnameDom).html('First name is required').removeClass('r_rfw_hide');
            }
            if (!lastNameValid) {
                this.jquery(lastnameDom).html('Last name is required').removeClass('r_rfw_hide');
            }
            if (!emailValid) {
                this.jquery(emailDom).html('Email is required').removeClass('r_rfw_hide');
            } else if (!validEmailFormat) {
                this.jquery(emailDom).html('Invalid Email format').removeClass('r_rfw_hide');
            }
        } else {
    ...
    private hideReviewerInfoErrors() {
        this.shadowRoot.querySelectorAll('.r_rfw_review_info_error')?.forEach((item: Element) => {
    ...
    private slide() {
        const mainContentWrapper = this.shadowRoot.querySelector('.r_rfw_main_content_wrapper') as HTMLElement;
    ...
    // Show footer
    private showFooter() {
        const footer = this.shadowRoot.querySelector('.r_rfw_footer_wrapper') as HTMLElement;
        if (footer) {
            this.jquery(footer).removeClass('r_rfw_hide');
            const forwardButton = this.shadowRoot.querySelector('.r_rfw_footer_forward_btn') as HTMLElement;
            if (this.isNeedToShowNext()) {
                const nextButtonName = this.getNextButtonName();
                if (forwardButton) {
                    this.jquery(forwardButton).removeClass('r_rfw_hide')
                    this.jquery(forwardButton).find('.r_rfw-button__text').text(nextButtonName)
                }
            } else {
                if (forwardButton) {
    ...
    private initRatingSection() {
        this.shadowRoot.querySelectorAll('.r_rfw_rating_icon').forEach((item: Element, index: number) => {
    ...
    private isMaxPhotoUploaded() {
        return (this.getUploadedPhotoCount() + this.review_details.extra.previous_photo_count) >= 5;
    }
    ...
    private initPhotoSection() {
        const addPhotoButtons = this.shadowRoot.querySelectorAll('.wd_add_photos_btn');
        addPhotoButtons.forEach((item: Element) => {
            this.jquery(item).on('click', () => {
                ...
                const fileInput = this.shadowRoot.querySelector('.r_frw_file_input') as HTMLElement;
                if (fileInput) {
    ...
    private handleFileUpload(e: Event) {
        ...
        reader.onloadend = () => {
            ...
            formData.append('_wp_nonce_key', this.review_form_store_config._wp_nonce_key);
    ...
    private addPhotoToList(attachment_id: string, attachment_url: string) {
        const imageContainerTemplate = this.shadowRoot.querySelector('.r_frw_img_container') as HTMLElement;
        const imageContainer = imageContainerTemplate.cloneNode(true) as HTMLElement;
        ...
        const list = this.shadowRoot.querySelector('.r_frw_photos_list') as HTMLDivElement;
        this.jquery(list).prepend(imageContainer);

        const photoContainer = this.shadowRoot.querySelector('.r_frw_view_photos_container') as HTMLElement;
    ...
    private showUploadButtons() {
        const emptyPhotoSection = this.shadowRoot.querySelector('.r_rfw_empty_photo_section') as HTMLElement;
        const photoContainer = this.shadowRoot.querySelector('.r_frw_view_photos_container') as HTMLElement;
    ...
    private initReviewContentSection() {
        const reviewContentInput = this.shadowRoot.querySelector('.r_rfw_review_content_text');
    ...
    private initReviewerInfoSection() {
        const firstNameInput = this.shadowRoot.querySelector('.r_rfw_reviewer_first_name');
        firstNameInput?.addEventListener('change', (event: Event) => {
            const target = event.target as HTMLInputElement;
            this.review_details.first_name = target.value;
        });

        const lastNameInput = this.shadowRoot.querySelector('.r_rfw_reviewer_last_name');
    ...
    private initFooter() {
        // Back button event
        this.shadowRoot.querySelectorAll('.r_rfw_footer_back_btn').forEach((item: Element) => {
            item.addEventListener('click', () => this.slidePrev());
        });

        // Forward button event
        this.shadowRoot.querySelectorAll('.r_rfw_footer_forward_btn, .r_rfw_continue_btn').forEach(
            (item: Element) => {
    ...
    private registerCloseIcon() {
        // Close dialog event
        this.shadowRoot.querySelector('.r_rfw_dialog_close_icon')?.addEventListener('click', () => {
    ...
    private submitLoading(submit: boolean = false) {
        ...
        submit ? this.jquery(submitButton).addClass('r_rfw-button--loading') : this.jquery(submitButton).removeClass('r_rfw-button--loading');
    }
    ...
    private async submit(): Promise<boolean> {
        ...
        try {
            ...
            const result = await this.jquery.ajax(this.review_form_store_config.ajax_url, {
                method: 'POST',
                data: data,
                contentType: 'application/x-www-form-urlencoded',
            });
