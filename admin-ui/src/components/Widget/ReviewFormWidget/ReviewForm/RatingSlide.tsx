...
const RatingSlide = ({rating, updateRating}: any) => {
    ...
    return (
        <div className={"r_rfw_rating_container"}>
            ...
            <p className={"r_rfw_rating_preview_title"}>{methods.getRatingTitle()}</p>
                        ...
                        return (
                            <div className={"r_rwf_rating_icons_wrapper_with_text"} key={index}>
                                <i key={index + '_' + rating}
                                   className={`r_rfw_rating_icon review-icon review  review_rating review_rating_${index + 1} ${index + 1 <= rating ? 'review-gem' : 'review-gem-outline'}`}
                                   onClick={() => updateRating(index + 1)}
                                ></i>
                                {(index === 0 || index === 4) && (
                                    <span
                                        className={"r_rwf_rating_icons_text"}>{index === 0 ? 'Dislike it' : 'Love it!'}</span>
                                )}
                            </div>)
                    })
                }
