    ...
    return (
        <div className={"r_rfw_photo_slide_container"}>
            ...
            <div className={"r_frw_photo_slide__text_container"}>
                ...
                <span className={"r_frw_photo_description"}>{widget.photos.description}</span>
            </div>
            <div className={"r_frw_add_photos_container"}>
                <div className={`r_frw_view_photos_container ${images.length == 0 ? 'r_rfw_hide' : ''}`}>
                    <div className={"r_frw_photos_list"}>
                        {images.slice(0, 5).map((imageSrc: string, index: number) => {
                                return (<div className={"r_frw_img_container"} key={index}>
                                                <span className={"review review-cross-icon r_frw_img_close_icon"}
                                                      onClick={() => {
                        ...
                        <div style={methods.getAddPhotosDivStyles()}
                             className={`r_rfw_add_photos_div wd_add_photos_btn ${images.length >= 5 ? ' r_rfw_hide' : ''}`}
                             onClick={handleButtonClick}
                        >
                            <i className={"review-plus frt-text-inherit review-icon review"}/>
                        </div>
                    </div>
                </div>
                <div className={`r_rfw_empty_photo_section ${images.length != 0 ? 'r_rfw_hide' : ''}`}>
                    <p className={"r_rfw_photo_discount_text"}>{widget.photos.discount_text}</p>
                    <button className={"r_frw_add_photos_btn wd_add_photos_btn"}
                            onClick={handleButtonClick}
