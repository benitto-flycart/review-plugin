...
const WidgetSidebar = ({
  ...
  const [whichLanguage, setWhichLanguage] = useState({
    label: "English",
    value: "en_US",
  });
  ...
  return (
    <>
      <div
        className={
          "frt-top-0 frt-col-span-1 frt-flex frt-flex-col frt-gap-2 frt-border-r frt-border-black-400"
        }
      >
        {showDropdownOptions() ? (
          <div
            className={
              "wd__sidebar_title  widget-header-title frt-sticky frt-bg-secondary frt-top-0 frt-flex frt-justify-between frt-p-4"
            }
          >
        ...
        {showDropdownOptions() ? (
          <div
            className={
              "wd_sidebar_options_container frt-divide-y frt-flex frt-flex-col frt-flex-1 frt-transition-transform frt-duration-1000"
            }
          >
            {settings?.options.map((option: any, index: number) => {
              return (
                <div
                  key={index}
                  className={
                    "wd_sidebar_options_container__option frt-p-3 frt-cursor-pointer frt-transition-transform frt-duration-1000"
                  }
                  onClick={() => {
                    updateWidgetFields((draftState: any) => {
                      draftState.show_setting =
                        draftState.show_setting == "" ? option.key : "";
                    });
                  }}
                >
                  <div
                    className={
                      "wd_sidebar_options_container__option_container frt-flex frt-flex-row frt-justify-between frt-items-center"
                    }
                  >
                    <div
                      className={
                        "wd_sidebar_option frt-flex frt-flex-row frt-items-center frt-gap-4"
                      }
                    >
                      <span>{option.icon}</span>
                      <span>{option.name}</span>
                    </div>
                    <span className={"wd_sidebar_option_goto_icon"}>
                      <ArrowRightIcon />
                    </span>
                  </div>
                </div>
      ...
      {widget.show_setting == "" && availableLanguages.length > 1 ? (
        <div
          className={
            "frt-flex frt-flex-col frt-gap-y-3 frt-absolute frt-bottom-7"
          }
        >
          <span className={"frt-mx-4"}>
            {" "}
            Choose language to apply previous style for this widget
          </span>
          <div className={"frt-flex frt-gap-5 frt-justify-center"}>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  className={"frt-w-[60%] !frt-justify-between"}
                >
                  {whichLanguage.label}
                  <ChevronDown />
                </Button>
            ...
            <Button disabled={loading} onClick={getPreviousStyles}>
              {loading ? <LoadingSpinner /> : null}Apply
            </Button>
          </div>
        </div>
      ) : null}
