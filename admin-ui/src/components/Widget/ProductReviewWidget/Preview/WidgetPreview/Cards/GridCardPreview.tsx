    ...
    return (
        ...
        >
            <div className={"r_pw_r_g_container--review-details"}>
                ...
                {review.images.length > 0 ? (<img src={review.images[0].src} className={"r_pw_r_g_container--review-details--image"}/>) : null}
                <div className={"r_pw_r_g_container--review-info"}>
                    <span className={"r_pw_r_g_container--review-info-name"}>{review.reviewer_name}</span>
                    {review.is_verified ? <span className={"r_pw_r_g_container--review-is-verified"}><Badge variant="outline">verified</Badge></span> : null}
            ...
            {review.replies?.length > 0 ? (<div className={"r_pw_r_g_container--reply-container"} style={getRepliesStyles(widget)}>
                    {review.replies.map((reply: any, index: number) => {
                        return (
                            <div key={index}>
                                <span className={"r_pw_r_g_container--reply--reviewer_name"}>{reply.reviewer_name}</span>
            ...
            <div className={"r_pw_r_g_container--product_container"}>
                ...
                <div className={"r_pw_r_g_container--product_container--product_name"}><span>{review.product.product_name}</span></div>
            </div>
        </div>
    )
}
