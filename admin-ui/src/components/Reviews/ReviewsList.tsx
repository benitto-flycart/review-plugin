...
export const ProductReview = <T extends ReviewRatingsPropType> ({reviewState,getReviews} : T) => {
    ...
    const [bulkActionLoading,setBulkActionLoading] = React.useState<boolean>(false)
                ...
                <div className="frt-flex frt-justify-start frt-gap-x-3 frt-items-center frt-mb-4">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" className={"frt-gap-x-2"}>{bulkActionState.label}
                                <ChevronDown/></Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent>
                            {
                    ...
                    <Button onClick={performBulkAction} disabled={bulkActionLoading} className={"frt-flex frt-gap-x-1"}> {bulkActionLoading ? <LoadingSpinner/> : ''}Apply</Button>
                </div>
            </div>

            <Card
                className="frt-bg-white frt-shadow-lg frt-max-w-4xl frt-mx-auto frt-p-6 frt-m-4 frt-flex frt-flex-col frt-gap-y-4">
                {
                    reviewState.reviews.length > 0 && reviewState.reviews.map((reviewData:any) => {
                        return <ReviewDetail review={reviewData} getReviews={getReviews} key={reviewData.id} bulkActionReviewIds={{val:bulkActionReviewIds,set:setBulkActionReviewIds}}/>
                    })
                }
            </Card>
        </div>
