...
const DiscountReminder = () => {
    const [savedDiscountReminders, setSavedDiscountReminders] = useState<any>([])
    ...
    const fetchReviewDiscountEmailSettings = () => {
        setListLoading(true)
        axiosClient.post('', {
            method: 'get_review_discount_email_settings_list',
            _wp_nonce_key: 'flycart_review_nonce',
            _wp_nonce: localState?.nonces?.flycart_review_nonce,
        }).then((response: any) => {
            let data = response.data.data;
            setSavedDiscountReminders(data.list);
        }).catch((error: any) => {
            toastrError("Server Error Occurred");
        }).finally(() => {
    ...
    useEffect(() => {
        fetchReviewDiscountRequest(values.language);
        fetchReviewDiscountEmailSettings();
    }, []);
                    ...
                    <div className="frt-h-full">
                        <div className='frt-flex frt-flex-col frt-gap-4'>
                            ...
                            {listLoading ? (
                                ...
                                <div className='frt-flex frt-flex-col frt-gap-4'>
                                    {savedDiscountReminders?.length > 0 ? (
                                        savedDiscountReminders?.map((item: any, index: any) => {
                                            return (
                                                <Card key={index}
