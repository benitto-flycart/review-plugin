...
const Orders = () => {
    const [orders, setOrders] = useState<TOrderList>(data.data);
                ...
                <div className="frt-container frt-mx-auto frt-p-6 frt-max-w-4xl">
                    ...
                    <div
                        ...
                        <div className="frt-flex frt-gap-4 frt-mb-4">
                            <Input type="email" value={filterState.search} onChange={(e)=>{
                                setFilterState({...filterState,search:e.target.value})
                            }} placeholder="Search by email" className="frt-w-full"/>
                            <Select
                            ...
                            >
                                ...
                                <SelectContent>
                                        {
                                            filterOrderStatus.map((filterOrder:any,index)=>{
                                                return < SelectItem key={index} value={filterOrder.value}>
                                                    {filterOrder.label}
                                                </SelectItem>
                                            })
                                        }
                            ...
                            >
                                ...
                                <SelectContent>
                                        {
                                            filterRange.map((range:any,index)=>{
                                                return < SelectItem key={index} value={range.value}>
                                                    {range.label}
                                                </SelectItem>
                                            })
                                        }
                        ...
                        {filterState.range=="custom" ?<div className=" frt-flex frt-gap-4 frt-mb-4 frt-w-full">
                            <div className="frt-relative frt-w-1/2">
                                <Input type="date" placeholder="Start date" className="frt-w-full" value={filterState.start_date} onChange={(e)=>{
                                    setFilterState({...filterState,start_date: e.target.value})
                                }}/>
                            </div>
                            <div className="frt-relative frt-w-1/2">
                                <Input type="date" placeholder="End date" className="frt-w-full" value={filterState.end_date} onChange={(e)=>{
                                    setFilterState({...filterState,end_date: e.target.value})
                                }}/>
                            </div>
                        </div> : null
                        }
                        <div className="frt-flex frt-justify-end">
                            <Button className="frt-bg-gray-900 frt-text-white frt-hover:bg-gray-800" onClick={()=>{
                                fetchOrders(true)
                            }}>Search</Button>
                        </div>
                    </div>
                            ...
                            !loading && searched && orders?.orders.length == 0 ? (
                                <div
                                    ...
                                    <div className="frt-mx-auto frt-my-auto frt-flex frt-flex-col frt-gap-5 frt-p-5">
                                        ...
                                        <div><span className="frt-text-lg frt-font-bold">The order you are looking for is not found</span>
                                        </div>
                                        <div>
                                            <p className="frt-text-sm">Uh oh, your order list is looking a little
                                                empty!
                                ...
                                !loading ? <>{(orders?.orders?.length ?? 0 > 0) && orders?.orders?.map((order: any, index: number) => {
                                    return (
                                        <React.Fragment key={index}>
                                            <OrderEntry order={order} fetchOrders={fetchOrders}
                                                        key={index}/>
                                        </React.Fragment>
                                    )
                                })}
